...
def _get_config_path() -> Optional[Path]:
    ...
    if sys.platform == "win32" or os.name == "nt":
        # Windows: use ~/.config/mineru2md/config.json (NOT %APPDATA%)
        return Path.home() / ".config" / "mineru2md" / "config.json"
    xdg_config = os.environ.get("XDG_CONFIG_HOME", "")
    if xdg_config:
        return Path(xdg_config) / "mineru2md" / "config.json"
    return Path.home() / ".config" / "mineru2md" / "config.json"
...
def get_token() -> str:
    ...
    Priority:
    1. MINERU_TOKEN environment variable (highest)
    2. ~/.config/mineru2md/config.json (all platforms)

    If config file does not exist, it will be created with a template.
    """
    # First try environment variable
    token = os.environ.get("MINERU_TOKEN")
    if token and token.strip():
        # Even with a valid token, ensure config file exists so the user
        # can persist the token there for future runs.
        _ensure_config_file(token)
        return token.strip()
    ...
    raise MinerUError(
        "MINERU_TOKEN environment variable is not set, and no valid token found in config. "
        f"Please set MINERU_TOKEN environment variable or add your token to {_get_config_path()}"
    )


def _ensure_config_file(token: str = "") -> None:
    """Ensure config directory and file exist. If file exists but mineru_token is empty, backfill with given token."""
    config_path = _get_config_path()
    if not config_path:
        return
    ...
    if config_path.exists():
        # File exists — check if mineru_token is empty and needs backfilling
        if not token:
            return
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                config = json.load(f)
            stored = config.get("mineru_token") or config.get("token") or ""
            if not stored.strip():
                # Backfill empty token
                config["mineru_token"] = token
                with open(config_path, "w", encoding="utf-8") as f:
                    json.dump(config, f, indent=2)
                print(f"[Config] Updated config file with token at: {config_path}")
        except (json.JSONDecodeError, IOError, TypeError):
            pass
        return
    try:
        with open(config_path, "w", encoding="utf-8") as f:
            json.dump({"mineru_token": token or ""}, f, indent=2)
        print(f"[Config] Created config file at: {config_path}")
    except IOError as e:
        print(f"[Config] Warning: Could not create config file: {e}")
  ...
  python mineru2md.py --file ./doc.pdf --print
    ...
    try:
        if args.files:
            ...
            # Ensure config file exists before routing (so token can be persisted regardless of API path)
            env_token = os.environ.get("MINERU_TOKEN", "")
            _ensure_config_file(env_token)
